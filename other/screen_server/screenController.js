let Scrn = require("./screen_model/index.js");
let scrn = new Scrn();
  ...
  switch (req.body.command) {
    case "up":
      if ((await scrn.up()) == "MOVING UP") {
        successHandler(
          req,
          res,
          "Success: The projector is successfully moving up"
        );
      } else {
    ...
    case "down":
      if ((await scrn.down()) == "MOVING DOWN") {
        successHandler(
          req,
          res,
          "Success: The screen is successfully moving down"
        );
      } else {
    ...
    case "stop":
      if ((await scrn.stop()) == "MANUALLY STOPPED") {
        successHandler(
          req,
          res,
          "Success: The projector has successfully stopped"
        );
      } else {
    ...
    default:
      failHandler(
        req,
        res,
        "Error: Unknown Command, try up, down, status, stop"
      );
  }
}
